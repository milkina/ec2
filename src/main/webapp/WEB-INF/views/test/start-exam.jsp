<div id="collapse_${test.pathName}" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="heading_${test.pathName}">
   <div class="panel-body">
     <form action="${pageContext.request.contextPath}/${pageLanguage=='ru'?'ru/':''}start-test/${test.pathName}"  method="post">
            ...
            <c:choose>
             ...
             <c:otherwise>
                <option value="${category.value.pathName}">
                  <spring:message code="module"/>&nbsp;${count} - ${category.value.name}
                </option>
             </c:otherwise>
             </c:choose>
