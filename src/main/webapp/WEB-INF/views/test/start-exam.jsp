  ...
  <form action="${pageContext.request.contextPath}/${pageLanguage=='ru'?'ru/':''}start-test/${test.pathName}" method="post" class="test-form" aria-label="Start test form">
      <div class="test-form-grid">
        <div class="test-field test-field--wide">
          ...
          <div class="field-error" id="CATEGORY_PATH_test_${test.pathName}_error" role="alert" aria-live="assertive" hidden>
            <spring:message code="select.category.required"/>
          </div>
        </div>
        <div class="test-field test-field--narrow">
          <label class="test-field-label" for="NUMBER_OF_QUESTIONS_${test.pathName}"><spring:message code="number.questions.label"/></label>
          <div class="test-qty">
            <button type="button" class="test-qty-btn dec button" aria-label="<spring:message code="decrease.number.questions"/>" aria-controls="NUMBER_OF_QUESTIONS_${test.pathName}">-</button>
            <input type="number" id="NUMBER_OF_QUESTIONS_${test.pathName}" value="25" name="NUMBER_OF_QUESTIONS" class="test-qty-input number-questions-input" min="5" max="100" aria-label="<spring:message code="number.questions.label"/>">
            <button type="button" class="test-qty-btn inc button" aria-label="<spring:message code="increase.number.questions"/>" aria-controls="NUMBER_OF_QUESTIONS_${test.pathName}">+</button>
          </div>
